...
interface StepDef {
  ...
  /** Función opcional para mostrar las opciones traducidas en la lista. */
...
const allSteps: StepDef[] = [
  ...
  },
  {
    ...
    renderOption: tQuestionnaireOption,
  },
  {
    key: "comunidadAutonoma",
    type: "autocomplete",
    getOptions: () => ccaaNames,
    renderOption: tCcaaByName,
...
const formatBudget = (value: number, maxLabel: string) =>
  value >= 60000 ? maxLabel : value.toLocaleString() + " €";
...
const GetInfoQuestionnaire = ({ onComplete, onBack }: GetInfoQuestionnaireProps) => {
  ...
  const budgetMax = t("questionnaire.budgetMax");
  ...
  const renderOpt = (o: string) => (current.renderOption ? current.renderOption(o) : o);

  const filteredOptions = current.type === "autocomplete" && searchText.length > 0
    ? currentOptions.filter((o) => renderOpt(o).toLowerCase().includes(searchText.toLowerCase()))
    : current.type === "autocomplete"
    ? []
    : currentOptions;

  // Sort autocomplete options by their localized label
  const sortedFiltered = current.type === "autocomplete"
    ? [...filteredOptions].sort((a, b) => renderOpt(a).localeCompare(renderOpt(b), i18n.language))
    : filteredOptions;
  ...
  const confirmSlider = () => {
    const value = `${formatBudget(sliderValue[0], budgetMax)} - ${formatBudget(sliderValue[1], budgetMax)}`;
    selectOption(value);
  };
    ...
    <section className="min-h-screen flex items-center bg-background py-20">
      <div className="container mx-auto px-4 max-w-lg">
        <button onClick={goBack} className="flex items-center gap-2 text-muted-foreground hover:text-foreground mb-6 transition-colors">
          <ArrowLeft className="h-4 w-4" /> {stepIndex > 0 ? t("common.previous") : t("common.back")}
        </button>

        <div className="bg-card rounded-2xl card-shadow p-8 md:p-10 space-y-8">
          <div className="space-y-2">
            <div className="flex justify-between text-sm text-muted-foreground">
              <span>{t("questionnaire.stepOf", { current: stepIndex + 1, total: totalSteps })}</span>
          ...
          {current.type === "masterSearch" && (
            <div className="space-y-3">
              ...
              {masterSearchText.trim() && (
                ...
                >
                  {t("questionnaire.searchFor", { query: masterSearchText.trim() })}
                </button>
              )}

              {!masterSearchText && !selected && (
                <p className="text-sm text-muted-foreground text-center py-2">
                  {t("questionnaire.searchHint")}
                </p>
              )}
            </div>
          )}
            ...
            <div className="space-y-6">
              <div className="text-center">
                <span className="text-3xl font-bold text-primary">
                  {formatBudget(sliderValue[0], budgetMax)} — {formatBudget(sliderValue[1], budgetMax)}
                </span>
              </div>
              <div className="px-2 space-y-4">
                ...
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>{formatBudget(current.sliderMin ?? 0, budgetMax)}</span>
                  <span>{formatBudget(current.sliderMax ?? 60000, budgetMax)}</span>
                </div>
              </div>
              <button
          ...
          {current.type === "autocomplete" && (
            <div className="space-y-2">
              <Input
                placeholder={t("questionnaire.searchPlaceholder")}
                value={searchText}
              ...
              {selected && searchText.length === 0 && (
                <p className="text-sm text-muted-foreground">
                  {t("questionnaire.selectedLabel")} <span className="font-semibold text-foreground">{renderOpt(selected)}</span>
                </p>
              )}
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {current.iconOptions.map((opt) => (
                ...
                >
                  ...
                  <span className="font-semibold text-foreground text-sm">{tQuestionnaireOption(opt.label)}</span>
                </button>
              ))}
            </div>
          )}
          ...
          {(current.type === "options" || current.type === "autocomplete") && (
            <div className="space-y-3 max-h-64 overflow-y-auto">
              {sortedFiltered.map((option) => (
                ...
                >
                  <span className="font-medium text-foreground">{renderOpt(option)}</span>
                </button>
              ))}
              {current.type === "autocomplete" && searchText.length > 0 && sortedFiltered.length === 0 && (
                <p className="text-sm text-muted-foreground text-center py-4">
                  {t("questionnaire.noSearchResults", { query: searchText })}
                </p>
              )}
              {current.type === "autocomplete" && searchText.length === 0 && !selected && (
                <p className="text-sm text-muted-foreground text-center py-4">
                  {t("questionnaire.startTyping")}
                </p>
              )}
            </div>
          )}
